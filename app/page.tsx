...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const loadCloud = async (userId: string, email: string | null) => {
      ...
      const [{ data: cloudProfiles, error: profileError }, { data: cloudAttempts, error: attemptError }, { data: cloudRewards, error: rewardError }, { data: cloudQuests, error: questError }, { data: adminRow }] = await Promise.all([
      ...
      if (profileError || attemptError || rewardError || questError) { setCloudStatus("error"); setCloudMessage("Cloud setupটি এখনো complete হয়নি। Updated SQL setup check করুন।"); return; }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      const questResult = questRows.length ? await cloud.from("ts_daily_quest_completions").upsert(questRows, { onConflict: "user_id,profile_id,quest_date,quest_index", ignoreDuplicates: true }) : { error: null };
      setCloudStatus(profileResult.error || attemptResult.error || rewardResult.error || questResult.error ? "error" : "synced");
    }, 700);
    return () => window.clearTimeout(timer);
  }, [profiles, attempts, redemptions, questCompletions, activeProfileId, cloudUserId, localReady]);
...
function SetupScreen({ isAdmin, onStart, onStartChapter, onSmartMission, onQuest, onBreak, onContentCheck, attempts, selectedClass, setSelectedClass, selectedSubject, setSelectedSubject, selectedChapter, setSelectedChapter, questionCount, setQuestionCount, difficulty, setDifficulty }: { isAdmin: boolean; onStart: () => void; onStartChapter: (chapterValue: string) => void; onSmartMission: () => void; onQuest: () => void; onBreak: () => void; onContentCheck: () => void; attempts: PracticeAttempt[]; selectedClass: ClassKey; setSelectedClass: (value: ClassKey) => void; selectedSubject: SubjectKey; setSelectedSubject: (value: SubjectKey) => void; selectedChapter: string; setSelectedChapter: (value: string) => void; questionCount: string; setQuestionCount: (value: string) => void; difficulty: Difficulty; setDifficulty: (value: Difficulty) => void }) {
