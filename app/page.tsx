...
import { sscStandardBangla, sscStandardBgs, sscStandardBiology, sscStandardChemistry, sscStandardEnglish, sscStandardHigherMath, sscStandardIct, sscStandardMath, sscStandardPhysics, sscStandardScience } from "@/data/class-9-10-standard-upgrade";
import { class9BoardUpgrade, class10BoardUpgrade } from "@/data/class-9-10-progression";
import { sscChallengeSet } from "@/data/class-9-10-challenge-set";
import { sscBoardAnalysisSet } from "@/data/class-9-10-board-analysis";
import { sscAuthoredDepth } from "@/data/class-9-10-authored-depth";
import { sscAuthoredFinal } from "@/data/class-9-10-authored-final";
...
const ModelTestCenter = dynamic(() => import("@/components/model-test-center").then((module) => module.ModelTestCenter), { loading: ToolLoading });
...
const baseCurriculumCatalog: Record<ClassKey, Partial<Record<SubjectKey, CatalogSubject>>> = {
  ...
  "9": { science: { label: "Science", questions: [...sscScience, ...sscScienceDepth, ...sscScienceFinal, ...sscStandardScience] }, math: { label: "Mathematics", questions: [...sscMath, ...sscMathDepth, ...sscMathFinal, ...sscStandardMath] }, english: { label: "English", questions: [...sscEnglish, ...sscEnglishDepth, ...sscEnglishFinal, ...sscStandardEnglish] }, bangla: { label: "Bangla", questions: [...sscBangla, ...sscBanglaDepth, ...sscBanglaFinal, ...sscStandardBangla] }, bgs: { label: "Bangladesh & Global Studies", questions: [...sscBgs, ...sscBgsDepth, ...sscBgsFinal, ...sscStandardBgs] }, physics: { label: "Physics", questions: [...sscPhysics, ...sscPhysicsFinal, ...sscStandardPhysics] }, chemistry: { label: "Chemistry", questions: [...sscChemistry, ...sscChemistryFinal, ...sscStandardChemistry] }, biology: { label: "Biology", questions: [...sscBiology, ...sscBiologyFinal, ...sscStandardBiology] }, higherMath: { label: "Higher Mathematics", questions: [...sscHigherMath, ...sscHigherMathFinal, ...sscStandardHigherMath] }, ict: { label: "ICT", questions: [...sscIct, ...sscIctFinal, ...sscStandardIct] } },
  "10": { science: { label: "Science", questions: [...sscScience, ...sscScienceDepth, ...sscScienceFinal, ...sscStandardScience] }, math: { label: "Mathematics", questions: [...sscMath, ...sscMathDepth, ...sscMathFinal, ...sscStandardMath] }, english: { label: "English", questions: [...sscEnglish, ...sscEnglishDepth, ...sscEnglishFinal, ...sscStandardEnglish] }, bangla: { label: "Bangla", questions: [...sscBangla, ...sscBanglaDepth, ...sscBanglaFinal, ...sscStandardBangla] }, bgs: { label: "Bangladesh & Global Studies", questions: [...sscBgs, ...sscBgsDepth, ...sscBgsFinal, ...sscStandardBgs] }, physics: { label: "Physics", questions: [...sscPhysics, ...sscPhysicsFinal, ...sscStandardPhysics] }, chemistry: { label: "Chemistry", questions: [...sscChemistry, ...sscChemistryFinal, ...sscStandardChemistry] }, biology: { label: "Biology", questions: [...sscBiology, ...sscBiologyFinal, ...sscStandardBiology] }, higherMath: { label: "Higher Mathematics", questions: [...sscHigherMath, ...sscHigherMathFinal, ...sscStandardHigherMath] }, ict: { label: "ICT", questions: [...sscIct, ...sscIctFinal, ...sscStandardIct] } },
};
const curriculumCatalog = Object.fromEntries(Object.entries(baseCurriculumCatalog).map(([classKey, subjects]) => [classKey, Object.fromEntries(Object.entries(subjects).map(([subjectKey, subject]) => {
  const progression = classKey === "9" ? class9BoardUpgrade : classKey === "10" ? class10BoardUpgrade : [];
  const classSpecific = progression.filter((question) => question.origin.includes(`Class ${classKey} ${subject.label} •`));
  const challenges = Number(classKey) >= 9 ? sscChallengeSet.filter((question) => question.origin.includes(`Classes 9–10 ${subject.label} •`)) : [];
  const boardAnalysis = Number(classKey) >= 9 ? sscBoardAnalysisSet.filter((question) => question.origin.includes(`Classes 9–10 ${subject.label} •`)) : [];
  const authoredDepth = Number(classKey) >= 9 ? sscAuthoredDepth.filter((question) => question.origin.includes(`Classes 9–10 ${subject.label} •`)) : [];
  const authoredFinal = Number(classKey) >= 9 ? sscAuthoredFinal.filter((question) => question.origin.includes(`Classes 9–10 ${subject.label} •`)) : [];
  return [subjectKey, { ...subject, questions: ensureMinimumQuestions([...subject.questions, ...classSpecific, ...challenges, ...boardAnalysis, ...authoredDepth, ...authoredFinal], { classKey, subjectKey }) }];
}))])) as Record<ClassKey, Partial<Record<SubjectKey, CatalogSubject>>>;
...
const coverage = (Object.keys(curriculumCatalog) as ClassKey[]).map((classKey) => {
  ...
  return { className: `Class ${classKey}`, status: "Live", subjects: `${subjects.length} subject tracks • ${questionTotal} practice questions` };
});
...
const difficultyCopy: Record<Difficulty, { label: string; hint: string }> = {
  easy: { label: "Foundation", hint: "মূল ধারণা ঝালাই" },
  medium: { label: "Medium", hint: "Concept check" },
  hard: { label: "Hard", hint: "Think deeper" },
};
const sscDifficultyCopy: Record<Difficulty, { label: string; hint: string }> = {
  easy: { label: "Board Core", hint: "Class-standard concept" },
  medium: { label: "Advanced", hint: "Application & calculation" },
  hard: { label: "Challenge", hint: "Multi-step analysis" },
};
const levelCopyFor = (classKey: ClassKey) => Number(classKey) >= 9 ? sscDifficultyCopy : difficultyCopy;
...
export default function Home() {
  const [screen, setScreen] = useState<"setup" | "quiz" | "result" | "break" | "quest" | "guardian" | "routine" | "model-test" | "grade-report" | "content-check">("setup");
  ...
  const startQuiz = () => {
    ...
    const difficultyPool = chapterPool.filter((question) => getDifficulty(question) === difficulty);
    let freshPool = difficultyPool.filter((question) => !seen.includes(question.id));
    if (!freshPool.length) {
      const chapterIds = new Set(difficultyPool.map((question) => question.id));
      window.localStorage.setItem(seenKey, JSON.stringify(seen.filter((id) => !chapterIds.has(id))));
      freshPool = difficultyPool;
    }
  ...
  const startChapterPractice = (chapterValue: string) => {
    ...
    const difficultyPool = chapterPool.filter((question) => getDifficulty(question) === difficulty);
    let freshPool = difficultyPool.filter((question) => !seen.includes(question.id));
    if (!freshPool.length) { const chapterIds = new Set(difficultyPool.map((question) => question.id)); window.localStorage.setItem(seenKey, JSON.stringify(seen.filter((id) => !chapterIds.has(id)))); freshPool = difficultyPool; }
    const requestedCount = questionCount === "all" ? freshPool.length : Number(questionCount);
    const nextQuestions = buildPracticeQuestions(freshPool, requestedCount, difficulty);
  ...
  return <><a className="skip-link" href="#main-content">মূল শেখার অংশে যান</a><main id="main-content" className="min-h-screen" tabIndex={-1}>
    <header className="site-header"><div className="shell header-inner">
      ...
      <nav className="main-nav" aria-label="প্রধান মেনু"><button className={`nav-link ${["setup", "quiz", "result"].includes(screen) ? "active" : ""}`} onClick={() => showSection("practice-builder")}>Smart Practice</button><button className="nav-link" onClick={() => showSection("curriculum-map")}>Mastery Map</button><button className={`nav-link ${screen === "model-test" ? "active" : ""}`} onClick={() => setScreen("model-test")}><Clock3 size={17} /> Model Test</button><button className={`nav-link ${screen === "routine" ? "active" : ""}`} onClick={() => setScreen("routine")}><CalendarDays size={17} /> Routine</button><button className={`nav-link ${screen === "break" ? "active" : ""}`} onClick={() => setScreen("break")}><Gamepad2 size={17} /> Break</button></nav>
    ...
    {screen === "model-test" && <ModelTestCenter classKey={selectedClass} subjects={Object.entries(curriculumCatalog[selectedClass]).filter((entry): entry is [string, CatalogSubject] => Boolean(entry[1])).map(([key, subject]) => ({ key, label: subject.label, questions: subject.questions }))} onBack={() => setScreen("setup")} />}
...
function SetupScreen({ isAdmin, loginStreak, activeProfileId, onStart, onStartChapter, onSmartMission, onQuest, onBreak, onRoutine, onModelTest, onGradeReport, onContentCheck, attempts, selectedClass, setSelectedClass, selectedSubject, setSelectedSubject, selectedChapter, setSelectedChapter, questionCount, setQuestionCount, difficulty, setDifficulty }: { isAdmin: boolean; loginStreak: number; activeProfileId: string; onStart: () => void; onStartChapter: (chapterValue: string) => void; onSmartMission: () => void; onQuest: () => void; onBreak: () => void; onRoutine: () => void; onModelTest: () => void; onGradeReport: () => void; onContentCheck: () => void; attempts: PracticeAttempt[]; selectedClass: ClassKey; setSelectedClass: (value: ClassKey) => void; selectedSubject: SubjectKey; setSelectedSubject: (value: SubjectKey) => void; selectedChapter: string; setSelectedChapter: (value: string) => void; questionCount: string; setQuestionCount: (value: string) => void; difficulty: Difficulty; setDifficulty: (value: Difficulty) => void }) {
  ...
  const freshCount = difficultyPool.filter((question) => !seenIds.includes(question.id)).length || difficultyPool.length;
  const availableCount = difficultyPool.length;
  ...
  return <div className="shell page-pad">
    <section className="welcome-strip"><div><p className="eyebrow">NCTB 2026 • বাংলা মাধ্যম</p><h1>{attempts.length ? "আবার শুরু করি, champion! 🌟" : "আজই তোমার শেখার যাত্রা শুরু হোক! 🚀"}</h1><p className="brand-tagline">শেখো। খেলো। এগিয়ে চলো। <span>Learn it. Play it. Own it.</span></p></div><div className="welcome-badges"><div className="streak-pill"><Sparkles size={18} /> টানা {loginStreak} দিন</div><button className="routine-quick-link" onClick={onRoutine}><CalendarDays size={17} /><span><strong>Study Routine</strong><small>আমার সপ্তাহ সাজাই</small></span><ArrowRight size={15} /></button><button className="routine-quick-link model-test-quick" onClick={onModelTest}><Clock3 size={17} /><span><strong>Model Test</strong><small>সময় ধরে পরীক্ষা দিই</small></span><ArrowRight size={15} /></button><div className="class-pill">Class 5–10 • Learning Live</div></div></section>
    ...
    <div className="dashboard-grid">
      <section className="practice-card" id="practice-builder">
        ...
        <div className="form-grid">
          ...
          <Field label="Questions"><Choice ariaLabel="Number of questions" value={effectiveQuestionCount} onValueChange={setQuestionCount} options={questionOptions} labels={[...numericOptions.map(String), `সব available • ${freshCount}`]} suffix=" questions" /></Field>
        ...
        <div className="coverage-note" key={`${selectedClass}-${selectedSubject}-${selectedChapter}-${difficulty}-${questionCount}`}><BookMarked size={18} /><div><strong>{availableCount ? `${actualQuestionCount}টি ${levelCopy[difficulty].label} প্রশ্ন নিয়ে practice ready!` : `এই chapter-এ ${levelCopy[difficulty].label} প্রশ্ন review চলছে`}</strong><span>{availableCount ? `আগে না-দেখা ${freshCount}টি প্রশ্ন বাকি। এই level শেষ হলে প্রশ্নগুলো নতুনভাবে মিশিয়ে revision শুরু হবে।` : "অন্য level বা সব অধ্যায় বেছে নাও—নতুন verified প্রশ্ন যোগ হলে এখানেই দেখা যাবে।"}</span></div></div>
        <div className="challenge-block"><label className="field-label">{Number(selectedClass) >= 9 ? "Board challenge level" : "Start with this level"}</label><div className="challenge-row">{(["easy", "medium", "hard"] as Difficulty[]).map((level) => <button type="button" key={level} className={difficulty === level ? "selected" : ""} onClick={() => setDifficulty(level)} aria-pressed={difficulty === level}><span>{levelCopy[level].label}</span><small>{levelCopy[level].hint}</small></button>)}</div></div>
      ...
      <aside className="side-column">
        ...
        <section className="student-tools" aria-label="Student planning tools"><button onClick={onRoutine}><CalendarDays size={20} /><span><strong>Routine Generator</strong><small>তোমার সময় অনুযায়ী weekly study plan</small></span><ArrowRight size={16} /></button><button onClick={onGradeReport}><ListChecks size={20} /><span><strong>My Grade Report</strong><small>Analysis ও practice reminder</small></span><ArrowRight size={16} /></button></section>
    ...
    <section className="curriculum-section coverage-compact"><div className="curriculum-heading"><div><p className="eyebrow">CLASS 5–10 COVERAGE</p><h2>Core learning ও SSC preparation</h2></div><span>{curriculumTotal.toLocaleString("en-US")} questions</span></div><div className="coverage-grid">{coverage.map((item) => <article key={item.className} className="live"><div><h3>{item.className}</h3><span>{item.status}</span></div><p>{item.subjects}</p></article>)}</div></section>
...
function QuizScreen({ cloudUserId, questions, classLabel, subjectLabel, difficulty, practiceMode, current, answer, revealed, onAnswer, onNext, onBack }: { cloudUserId: string | null; questions: ScienceQuestion[]; classLabel: ClassKey; subjectLabel: string; difficulty: Difficulty; practiceMode: "standard" | "focus"; current: number; answer: number | null; revealed: boolean; onAnswer: (i: number) => void; onNext: () => void; onBack: () => void }) {
  ...
  const levelCopy = levelCopyFor(classLabel);
  const learningPrompt = /Math|Physics|Chemistry/.test(subjectLabel)
    ? "সূত্র বা নিয়মটির প্রতিটি মান কোথা থেকে এলো—একবার নিজের ভাষায় বলো।"
    : /English|Bangla/.test(subjectLabel)
      ? "একই নিয়ম ব্যবহার করে নিজের একটি ছোট example বানাও।"
      : "কারণ থেকে ফলটি কীভাবে এলো—এক লাইনে নিজের ভাষায় বলো।";
  ...
  return <div className="quiz-shell"><div className="quiz-topline"><button className="back-link" onClick={onBack}><ArrowLeft size={18} /> ফিরে যাই</button><span>Class {classLabel} • {subjectLabel} • {practiceMode === "focus" ? "Smart Review" : levelCopy[difficulty].label} • NCTB 2026</span></div><Progress value={((current + 1) / questions.length) * 100} className="quiz-progress" aria-label={`প্রশ্ন ${current + 1} এর ${questions.length}`} /><div className="question-count"><strong>প্রশ্ন {current + 1} / {questions.length}</strong><span>•</span> {practiceMode === "focus" ? "আবার শেখা" : levelCopy[difficulty].label} <span>•</span> {question.chapter} <span>•</span> {question.topic}</div>
    <section className="question-card"><div className="question-card-head"><div className="question-icon"><BookOpen size={24} /></div><div className="question-card-tools"><a className="question-source" href={question.sourceUrl} target="_blank" rel="noreferrer" title={question.origin} aria-label={`এই প্রশ্নের NCTB source দেখুন: ${question.origin}`}><BookMarked size={15} /> Source</a><button className="report-question" onClick={() => { setReportState("idle"); setReportOpen(true); }}><Flag size={15} /> প্রশ্নে সমস্যা?</button></div></div><h1>{question.prompt}</h1><p className="answer-instruction">একটি উত্তর বেছে নিয়ে <strong>“উত্তর মিলাই”</strong> চাপো।</p><div className="option-list">{question.options.map((option, index) => { const isCorrect = revealed && index === question.answer; const isWrong = revealed && answer === index && index !== question.answer; return <button key={option} onClick={() => onAnswer(index)} disabled={revealed} aria-pressed={answer === index} className={`${answer === index ? "chosen" : ""} ${isCorrect ? "correct" : ""} ${isWrong ? "wrong" : ""}`}><span className="option-letter">{String.fromCharCode(65 + index)}</span><span>{option}</span>{isCorrect && <Check size={20} />}</button>; })}</div>
      {revealed && <div className={`explanation ${correct ? "success" : "review"}`} role="status" aria-live="polite"><Lightbulb size={22} /><div><strong>{correct ? "ঠিক হয়েছে! +10 Power Stars ⭐" : `সঠিক উত্তর: ${question.options[question.answer]}`}</strong>{!correct && answer !== null && <span className="answer-why">তুমি বেছে নিয়েছিলে: <b>{question.options[answer]}</b>—এখন দুটো option-এর পার্থক্য মিলিয়ে দেখো।</span>}<p><b>কেন?</b> {question.explanation}</p><div className="learning-takeaway"><Sparkles size={15} /><span><b>নিজে যাচাই করো:</b> {learningPrompt}</span></div></div></div>}
